...
class CartController extends Controller
{
    public function index()
    {
        if($stt=0){
            ...
            return view('pages.cart',compact('qtysp'),compact('subtotal'))->with('products',$products)->with('content',$content)->with('total',$total)->with('qty',$qty);     
        }           
    }
     public function getAddtoCart($id){
            $stt=1;
            $product = Product::find($id);
            $insert = Cart::add(['id' => $product->id, 'name' => $product->name, 'qty' => 1, 'price' => $product->price, 'image' =>  $product->image]);
    ...
    public function getCheckout(){
        ...
        $qtysp=Cart::content()->count();
        $subtotal=Cart::subtotal();
        return view('home',compact('content'),compact('products'),compact('qtysp'),compact('subtotal'))->with('total',$total)->with('qty',$qty);
    }
